...
const TimelinePoint = ({ point, diff }: { point: WorkTimelinePoint, diff: number }) => {
  ...
  return (
    <group position={point.point} scale={window.innerWidth < 768 ? 0.35 : 0.6}>
...
const Timeline = ({ progress }: { progress: number }) => {
  ...
  useFrame((_, delta) => {
    if (isActive) {
      ...
      camera.position.x = THREE.MathUtils.damp(camera.position.x, (window.innerWidth < 768 ? -1 : -2) + position.x, 4, delta);
