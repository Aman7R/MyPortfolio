...
const Experience = () => {
  ...
  const getTitle = () => {
    ...
    return title.split('').map((char, i) => {
      const diff = window.innerWidth < 768 ? 0.4 : 0.8;
      return (
  ...
  return (
    <group position={[0, -41.5, 12]} rotation={[-Math.PI / 2, 0 ,-Math.PI / 2]}>
      <group rotation={[0, 0, Math.PI / 2]}>
        <group ref={titleRef} position={window.innerWidth < 768 ? [-1.8, 2, -2] : [-3.6, 2, -2]}>
          {getTitle()}
        </group>
        ...
        <group position={[0, -1, 0]} ref={groupRef}>
          <GridTile title='WORK AND EDUCATION'
            ...
            color='#b9c6d6'
            textAlign='left'
            position={new THREE.Vector3(window.innerWidth < 768 ? -1 : -2, 0, window.innerWidth < 768 ? 0.4 : 0)}>
            <Work/>
          </GridTile>
          <GridTile title='SIDE PROJECTS'
            ...
            color='#bdd1e3'
            textAlign='right'
            position={new THREE.Vector3(window.innerWidth < 768 ? 1 : 2, 0, 0)}>
            <Projects/>
          </GridTile>
        </group>
      </group>
